...
function dominantBadgeStyle(label: string) {
  if (label === "Negatif") return "bg-red-50 border border-red-100 text-red-700";
  if (label === "Positif") return "bg-emerald-50 border border-emerald-100 text-emerald-700";
...
export default function StrategySection({
  ...
  const isOverridden = selectedAngle !== analysis.recommended_strategy.label;

  return (
    <section id="strategy-section" className="space-y-10 border-t border-zinc-200/80 pt-16 scroll-mt-20">
      ...
      <div className="space-y-2">
        ...
        <h2 className="font-serif text-2xl md:text-[2rem] tracking-tight text-zinc-900 font-extrabold">
          ...
          <span className="text-teal-600 font-serif italic">{analysis.keyword}</span>?
        </h2>
        <p className="text-sm leading-relaxed text-zinc-500 max-w-lg font-medium">
          Analisis sentimen dari {analysis.total_comments} komentar publik yang relevan dengan topik ini.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 border border-zinc-200/80 rounded-2xl bg-white shadow-sm flex flex-col justify-between min-h-[260px]">
          <div className="flex justify-between items-start flex-wrap gap-3">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-bold ${dominantBadgeStyle(dominant)}`}>
              {dominant}
            </span>
          </div>
          ...
          <div className="flex items-center justify-around gap-6 flex-wrap my-6">
            <div className="relative w-32 h-32 shrink-0">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r={radius} stroke="#f4f4f5" strokeWidth="12" fill="none" />
                <circle
                  cx="50" cy="50" r={radius} stroke="#ef4444" strokeWidth="12" fill="none"
                  strokeDasharray={`${negativeLength} ${circumference}`} strokeDashoffset="0" strokeLinecap="round"
                />
                <circle
                  cx="50" cy="50" r={radius} stroke="#f59e0b" strokeWidth="12" fill="none"
                  strokeDasharray={`${neutralLength} ${circumference}`}
                  strokeDashoffset={`${-negativeLength}`} strokeLinecap="round"
                />
                <circle
                  cx="50" cy="50" r={radius} stroke="#10b981" strokeWidth="12" fill="none"
                  strokeDasharray={`${positiveLength} ${circumference}`}
                  strokeDashoffset={`${-(negativeLength + neutralLength)}`} strokeLinecap="round"
                />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-xl font-extrabold text-zinc-900 leading-none">{analysis.total_comments}</span>
        ...
        <div className="p-6 border border-zinc-200/80 rounded-2xl bg-white shadow-sm flex flex-col justify-between min-h-[260px]">
          <div>
            <h3 className="text-sm font-bold text-zinc-900">Frasa Dominan</h3>
          ...
          <div className="space-y-4 my-4 flex-1 flex flex-col justify-center">
            {dominantPhrases.length === 0 ? (
              <p className="text-xs text-zinc-400 font-medium">Belum ada frasa dominan untuk kategori ini.</p>
            ) : (
              dominantPhrases.map((p, idx) => (
                <div key={`${p.keyword}-${idx}`} className="space-y-1">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-bold text-zinc-700">{idx + 1}. {p.keyword}</span>
                    <span className="text-zinc-400 font-bold">{p.count}%</span>
                  </div>
                  <div className="w-full h-1.5 rounded-full bg-zinc-100 overflow-hidden">
                    <div className="h-full rounded-full bg-teal-600" style={{ width: `${p.count}%` }} />
                  </div>
                </div>
              ))
            )}
      ...
      <form onSubmit={onGenerateIdeas} className="space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-zinc-400 font-medium">
            Sistem sudah menandai strategi yang paling sesuai dengan data di atas. Anda tetap bisa memilih strategi lain.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {ANGLE_OPTIONS.map((angle) => {
            ...
            const isRecommended = angle.id === analysis.recommended_strategy.label;
            return (
              <button
                type="button"
                key={angle.id}
        ...
        <div className="rounded-2xl border border-teal-100 bg-teal-50/40 p-6">
          <span className="text-[11px] font-bold uppercase tracking-wider text-teal-700">Reason</span>
          <p className="text-sm text-zinc-600 leading-relaxed mt-2 max-w-3xl">{reasonText}</p>

          {isOverridden && (
            <p className="text-xs text-zinc-500 leading-relaxed mt-3 max-w-3xl">
              Anda memilih strategi <span className="font-bold text-zinc-700">{selectedAngle}</span>, berbeda dari
              rekomendasi sistem (<span className="font-bold text-zinc-700">{analysis.recommended_strategy.label}</span>).
            </p>
          )}
        </div>
